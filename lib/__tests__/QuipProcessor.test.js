...
const defaultOptions = {
    ...
    quipApiURL: "URL",
    fileExistsCallback: jest.fn(() => false)
};
...
describe('constructor tests', () => {
    test('init paramteres', async () => {
        ...
        expect(quipProcessor.fileExistsCallback).toBe(defaultOptions.fileExistsCallback);
...
describe('methods tests', () => {
    ...
    describe('_processFile', () => {
        ...
        test('skip download if file exists', async () => {
            quipProcessor.options.fileExistsCallback.mockReturnValue(true);
            const returnedValue = await quipProcessor._processFile('HTML', fileInfo, '/aaa/', false);
            expect(quipProcessor.quipService.getBlob).not.toHaveBeenCalled();
            expect(returnedValue).toBe('HTML-blobs/FILE_NAME_SANITIZED');
        });
    ...
    describe('_processDocumentThreadDocx', () => {
        ...
        test('skip when file exists', async () => {
            quipProcessor.options.fileExistsCallback.mockReturnValue(true);
            await quipProcessor._processDocumentThreadDocx(thread, '/aaa/');
            expect(quipProcessor.quipService.getDocx).not.toHaveBeenCalled();
            expect(quipProcessor.saveCallback).not.toHaveBeenCalled();
        });
    });
    ...
    describe('_processDocumentThreadXlsx', () => {
        ...
        test('skip when file exists', async () => {
            quipProcessor.options.fileExistsCallback.mockReturnValue(true);
            await quipProcessor._processDocumentThreadXlsx(thread, '/aaa/');
            expect(quipProcessor.quipService.getXlsx).not.toHaveBeenCalled();
            expect(quipProcessor.saveCallback).not.toHaveBeenCalled();
        });
    });
