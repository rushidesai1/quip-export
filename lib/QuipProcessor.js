...
class QuipProcessor {
    constructor (quipToken, saveCallback = ()=>{}, progressCallback = ()=>{}, phaseCallback = ()=>{}, options={}) {
        ...
        this.fileExistsCallback = options.fileExistsCallback || (() => false);
    ...
    async _processDocumentThreadDocx(quipThread, path) {
        ...
        if(this.fileExistsCallback(fileName, path)) {
            return;
        }
    ...
    async _processFile(html, file, path, asImage=false) {
        let fileName;
        if(!asImage && file.fileName) {
            fileName = sanitizeFilename(file.fileName.trim());
            if(this.fileExistsCallback(fileName, `${path}blobs`)) {
                return html.replace(file.replacement, `blobs/${fileName}`);
            }
        }
